...
def test(model, test_dataset, test_loader, output_filename, result_dir=None):
    ...
    timeFile = open("test-time-log.txt", "w")	
    timeFile.write('Average time:\n')	
    timeFile.write('batch:%0.6f\n' % fw_time_meter.avg)	
    timeFile.write('avg_per_object:%0.6f\n' % (fw_time_meter.avg / load_batch_size))	
    timeFile.write('avg_per_image:%.6f\n' % (fw_time_meter.avg * len(test_loader) / num_images))	
    timeFile.close()
    #MODIFICATION END - PRINT TIME METRICS

    logging.info('Average time:')
    logging.info('batch:%06f' % fw_time_meter.avg)
    logging.info('avg_per_object:%0.6f' % (fw_time_meter.avg / load_batch_size))
    logging.info('avg_per_image:%.6f' % (fw_time_meter.avg * len(test_loader) / num_images))
